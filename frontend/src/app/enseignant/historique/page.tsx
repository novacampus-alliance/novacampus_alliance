  ...
  return (
    ...
    >
      <section aria-label="Historique des classes">
        <h2 className="mb-2 text-sm font-semibold text-gray-700">Classes passées</h2>
        {sessions === null ? (
            ...
            <table aria-label="Historique des classes" className="min-w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50 text-left text-xs font-semibold uppercase tracking-wider text-gray-700">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">Étudiants</th>
                  <th scope="col" className="px-4 py-3">Moyenne</th>
                  <th scope="col" className="px-4 py-3">Réussite</th>
                </tr>
              </thead>
                ...
                {filteredSessions.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td className="px-4 py-3">
                      <div className="font-mono text-xs text-gray-600">{s.courseCode}</div>
      ...
      <section aria-label="Historique des étudiants" className="mt-8">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-sm font-semibold text-gray-700">Historique étudiants</h2>
          <input
            type="search"
        ...
        {students === null ? (
          ...
          <p className="py-4 text-sm text-gray-500">
            {q
              ...
              : 'Aucun historique étudiant disponible.'}
          </p>
        ) : (
              ...
              <Card key={s.id} className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-gray-600">{s.email}</div>
                  </div>
                </div>
                  ...
                  {s.courses.map((c, i) => (
                    ...
                    >
                      <span className="min-w-0">
                        <span className="font-mono text-xs text-gray-600">{c.courseCode}</span>{' '}
                        <span className="text-gray-900">{c.courseName}</span>
                        <span className="ml-1 text-xs text-gray-600">· {c.semester}</span>
                      </span>
                      <span className="shrink-0 text-xs text-gray-700">
                        ...
                        · Présence {formatPercent(c.attendanceRate)}
                      </span>
                    </li>
                  ))}
                </ul>
